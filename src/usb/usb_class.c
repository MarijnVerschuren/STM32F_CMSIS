 ...
 * imported functions
 * */
extern void open_IEP(uint8_t ep_num, uint16_t mps, EP_type_t type);
extern void open_OEP(uint8_t ep_num, uint16_t mps, EP_type_t type);
extern void open_EP(uint8_t ep_num, uint16_t mps, EP_type_t type);
extern void close_IEP(uint8_t ep_num);
extern void close_OEP(uint8_t ep_num);
extern void close_EP(uint8_t ep_num);
...
void HID_keyboard_init() {
	...
	open_IEP(HID_keyboard->iep_num, HID_KEYBOARD_MPS, EP_TYPE_INTERRUPT);
	// TODO: state? @406 [usbd_hid_keyboard.c]
}
void HID_keyboard_de_init() {
	close_IEP(HID_keyboard->iep_num);
}
void HID_keyboard_setup() {
	// TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
}
void HID_keyboard_IN(uint8_t epnum) {
	// TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
}
...
USB_handle_t* fconfig_USB_handle(USB_class_type_t class_type, uint8_t iep_num, uint8_t oep_num, uint32_t descriptor_size) {
	...
	if (!USB_handle) {  // create status if not available
		USB_handle = malloc(sizeof(USB_handle_t));
	} else {
		if (USB_handle->descriptor) {
			if(USB_handle->descriptor->device)					{ free(USB_handle->descriptor->device); }
			if(USB_handle->descriptor->lang_ID_string)			{ free(USB_handle->descriptor->lang_ID_string); }
			if(USB_handle->descriptor->manufacturer_string)		{ free(USB_handle->descriptor->manufacturer_string); }
			if(USB_handle->descriptor->product_string)			{ free(USB_handle->descriptor->product_string); }
			if(USB_handle->descriptor->serial_string)			{ free(USB_handle->descriptor->serial_string); }
			if(USB_handle->descriptor->configuration_string)	{ free(USB_handle->descriptor->configuration_string); }
			if(USB_handle->descriptor->interface_string)		{ free(USB_handle->descriptor->interface_string); }
			free(USB_handle->descriptor);
		} if (USB_handle->class) {
			if (USB_handle->class->descriptor)					{ free(USB_handle->class->descriptor); }
			free(USB_handle->class);
		}
		for (uint8_t i = 0; i < 16U; i++) {
			if (USB_handle->IEP[i]) { free(USB_handle->IEP[i]); }
			if (USB_handle->OEP[i]) { free(USB_handle->OEP[i]); }
		}
	}
